...
**Nota de corrección (2026-07-23, auditoría final de cierre de la Fase 6):** esta sección originalmente pedía que el Compositor "aumentara el peso de cercanía" para compensar una señal de afinidad escasa — una redacción que, sin que nadie lo notara durante el diseño técnico del Bloque 4, contradecía directamente el principio permanente ya registrado más abajo de que "la cercanía geográfica actúe siempre como restricción base, nunca como señal de puntuación". La cercanía no tiene ni puede tener un peso ajustable: es un filtro binario (`geo_eligible()`), no una de las seis entradas ponderadas del Compositor. El texto de abajo describe ahora, con precisión, lo que el sistema realmente hace — que es honesto y ya cumple el espíritu original (nunca fabricar personalización artificial), aunque no mediante el mecanismo explícito que este párrafo pedía.

No existe un **estado de señal insuficiente detectado explícitamente** con un comportamiento propio distinto: lo que existe es una consecuencia honesta del diseño ya construido. `candidatos_afinidad()` nunca fabrica candidatos — una persona con poco o ningún historial simplemente recibe menos (o ningún) candidato de Afinidad, nunca datos inventados para simular una personalización que todavía no existe. El Compositor, mediante colas justas ponderadas, no necesita ningún caso especial para esto: si Afinidad no aporta candidatos, su cupo relativo simplemente no se ocupa, y las cinco entradas restantes (Novedad, Diversidad, Equidad, Serendipia, Editorial) absorben esa capacidad de forma proporcional a sus propios pesos entre sí — nunca específicamente hacia "cercanía" (que sigue sin tener peso alguno) ni mediante una reponderación explícita que alguien decida en el momento.

La transición entre poca y suficiente señal de afinidad es, en consecuencia, gradual por construcción, no por un mecanismo adaptativo dedicado: el volumen de candidatos de Afinidad crece con la evidencia acumulada (`affinity_profile()`, Bloque 1) de forma continua, nunca en un salto de umbral — un cambio abrupto de personalidad del Feed en un punto fijo se sentiría como una inconsistencia, no como una mejora, y el diseño ya construido evita eso sin necesitar ningún caso especial adicional.
