...
12. **Nunca confunde "cuenta eliminada" con "comentario eliminado" — son dos estados distintos y permanentes, nunca intercambiables.** (Fase 5B, Bloque 3, `PROJECT.md`). Una **cuenta eliminada** conserva el contenido del comentario (el texto sigue existiendo y siendo legible) pero pierde la identidad de su autor, que pasa a mostrarse como "Cuenta eliminada" — un actor de sistema que puede agregar comentarios de varias cuentas eliminadas distintas sin que eso implique en ningún caso la misma autoría real. Un **comentario eliminado** es lo opuesto: conserva únicamente la estructura de la conversación (que existió, cuándo, en respuesta a qué) pero su contenido se destruye de forma permanente y deliberada — nunca recuperable, ni siquiera mediante herramientas administrativas normales, por respeto pleno a la decisión de quien eliminó su propio comentario y al principio de minimización de datos. Si la Guía IA llegara a razonar sobre el historial de comentarios de un contenido, debe tratar estos dos estados como semánticamente opuestos: uno es "no sé quién lo dijo, pero esto es lo que se dijo"; el otro es "sé que algo se dijo aquí, pero ya no existe". Confundirlos — por ejemplo, tratando un comentario eliminado como si aún tuviera contenido recuperable, o asumiendo que dos comentarios de "Cuenta eliminada" en el mismo contenido pertenecen a la misma persona — sería un error de razonamiento, no una simplificación aceptable.
...
- Nunca debe tratar "cuenta eliminada" (contenido conservado, autor anonimizado) y "comentario eliminado" (contenido destruido de forma permanente, estructura conservada) como si fueran el mismo estado — ver el Principio no negociable 12.
