...
**Nota de corrección (2026-07-23, cierre de la Fase 6):** esta sección recomendaba originalmente evolucionar el feed hacia un modelo híbrido de puntuación (una sola fórmula sumando afinidad + peso editorial + peso de promoción + decaimiento de frescura). Esa recomendación quedó reemplazada, antes de implementarse, por un diseño deliberadamente distinto — ver `FASE6_FILOSOFIA_DESCUBRIMIENTO.md` y `FASE6_CONTRATO_ARQUITECTONICO.md` para la autoridad completa. Lo construido (Fase 6, Bloques 1-4) no es una fórmula de puntuación: es una composición de seis entradas co-iguales — **Motor de Afinidad** (candidatos por interés real), **Motor de Garantías** (novedad, diversidad, equidad y serendipia — cuatro carriles que nunca llegan a cero, actuando junto a la afinidad, nunca como corrección posterior), **Motor Editorial** (criterio humano, inserción paralela, nunca fusionado con el cálculo de afinidad) y el **Compositor del Feed** (integra las seis por proporciones relativas mediante Weighted Fair Queuing, con anti-monopolio transversal, sin ninguna fórmula de puntuación combinada). `editor_pick` quedó legacy — la curaduría editorial real vive en `editorial_selections`. El techo de contenido patrocinado (§52) permanece como una restricción dura no negociable que el Compositor debe respetar siempre, construida vacía de contenido real hasta la Fase 11: la regla de oro sigue siendo que ningún peso de promoción pagada puede desplazar contenido relevante no pagado, exactamente el mismo principio de confianza descrito en (b) de §2, ahora aplicado a un compositor sin fórmula en vez de a una que nunca se construyó.
