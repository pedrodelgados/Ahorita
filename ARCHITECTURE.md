...
Se proponen seis abstracciones centrales. Esta sección es conceptual (entidades y su propósito) — las relaciones formales están en §11, y **no se incluye ni se debe incluir ningún esquema SQL ni de base de datos aquí**, por instrucción explícita de esta fase.

**Actor.** Cualquier entidad que puede tener seguidores, publicar contenido y ser mencionada: una Persona o un Negocio/Organizador. Hoy el sistema tiene `profiles` (personas) y `businesses` (negocios) como conceptos separados sin una identidad común — la recomendación es que ambos se traten como variantes de un mismo concepto de Actor con capacidades distintas (una Persona no vende ni se verifica como negocio; un Negocio sí), para que Seguir, Mencionar, y Mostrar-perfil funcionen igual sin import bordar lógica duplicada por tipo.

**Lugar.** Una ubicación física persistente en la ciudad — puede o no estar asociada a un Negocio (un parque público es un Lugar sin negocio dueño; un restaurante es un Lugar con un Negocio asociado). Este concepto ya existe (`places`) y se mantiene esencialmente igual, pero se recomienda que todo Negocio tenga automáticamente un Lugar asociado (hoy son conceptos con solapamiento parcial: `businesses` tiene su propia lat/lng en vez de referenciar un `place`), para que el mapa, la búsqueda y el check-in QR (§44) tengan una sola fuente de verdad geográfica sin importar si el punto en el mapa es un negocio, un espacio público, o un punto de encuentro de un evento sin sede fija.

**Publicación (Contenido).** La abstracción central de §8: cualquier pieza de contenido con fecha de vigencia, autor (un Actor), posiblemente un Lugar asociado, medios (foto/video), y un ciclo de vida editorial (borrador/publicado/oculto/finalizado/cancelado — patrón que ya existe para eventos y se generaliza aquí). Subtipos: Evento (tiene fecha de inicio obligatoria), Promoción (tiene vigencia y condición de canje, ver §17), Publicación regular (un post social sin fecha de vigencia dura), Historia (vigencia de 24 horas, ver §35), Video/Reel (formato, no vigencia — ver §34).

**Interacción.** Cualquier acción de un Usuario sobre una Publicación o un Actor: me gusta, comentario, guardado, seguimiento. Ya modelada parcialmente (patrón `target_type`/`target_id` en `post_likes`) — se recomienda generalizar el mismo patrón a comentarios y guardados, que hoy están fragmentados en tablas específicas por tipo (`event_comments`, `saved_places`, `saved_events`).

**Verificación.** Una solicitud de confianza sobre un Actor (negocio u organizador), con evidencia, estado (pendiente/aprobada/rechazada/vencida), revisor y fecha de vencimiento — no un simple booleano. Detalle en §15.

**Ciudad.** La dimensión de escopo geográfico mencionada en el principio (a) de §2. Cada Lugar, Evento y Negocio pertenece a una Ciudad. Hoy el sistema no tiene esta dimensión porque asume implícitamente "todo es Cuenca" — se recomienda introducirla desde ahora, con un único valor poblado ("Cuenca"), precisamente para que el día que se evalúe expansión (Horizonte 3, §3) no requiera una migración estructural de todas las tablas existentes, solo poblar un valor que ya tiene dónde vivir.
...
- **Actor-Negocio** (evoluciona `businesses`): identidad de negocio, categoría, horario, contacto, estado de verificación (referenciando la nueva entidad Verificación en vez de un campo de estado plano).
...
- **Publicación** (generaliza `events` y, a futuro, sustituye la necesidad de tablas paralelas para promociones/posts/historias): título, descripción, medios, autor, lugar opcional, categoría, subtipo, fechas de vigencia, estado editorial, métricas desnormalizadas (ya existe el patrón de `likes_count`/`comments_count` mantenido por triggers — se mantiene).
- **Interacción** (generaliza `post_likes` y absorbe conceptualmente lo que hoy son `event_comments`, `saved_places`, `saved_events`): tipo de interacción, actor que interactúa, publicación o actor objetivo.
- **Verificación** (nuevo): actor solicitante, evidencia, estado, revisor, vigencia.
- **Ciudad** (nuevo): nombre, límites geográficos aproximados (para geocercas y sesgo de búsqueda — ya se usa informalmente un "viewbox" de Cuenca en la búsqueda de ubicación construida en la fase anterior).
- **Categoría/Canal** (continúa `channels`): taxonomía compartida entre Lugares y Publicaciones.
- **Solicitud de moderación/reporte** (nuevo, ver §56): contenido o actor reportado, motivo, estado, resolución.
- **Sesión de Guía IA** (nuevo formalmente, aunque ya existe informalmente vía la función de borde actual): historial de interacción con la IA, para aprendizaje de preferencias (§40) con consentimiento explícito.
- **Token QR** (nuevo, ver §41): código, tipo de acción (check-in, canje de promoción, validación de entrada), entidad referenciada, estado de uso, vigencia.
...
Un **Actor-Persona** sigue a otros Actores (personas o negocios) — una sola relación de "seguimiento" sirve para ambos casos si Actor es una abstracción común (§9), en vez de necesitar una tabla de "seguir personas" y otra futura de "seguir negocios". Un **Actor-Negocio** u **Organizador** tiene una **Verificación** asociada que determina si puede publicar con la insignia de confianza. Un **Actor-Negocio** está asociado opcionalmente a un **Lugar** (opcionalmente, porque un Organizador de eventos puede no tener uno). Una **Publicación** pertenece siempre a un **Actor** autor, opcionalmente a un **Lugar**, siempre a una **Categoría**, y vive dentro de una **Ciudad** (heredada de su Lugar si tiene uno, o asignada directamente si no). Una **Interacción** conecta siempre un **Actor-Persona** (quien interactúa) con una **Publicación** o un **Actor** (el objetivo). Un **Token QR** referencia una **Publicación** (para validar una entrada o canjear una promoción) o un **Lugar** (para check-in) y opcionalmente un **Actor-Persona** (a quién se le emitió, si aplica). Una **Solicitud de moderación** referencia una **Publicación** o un **Actor** reportado, y un **Actor** (rol Editor/Curador o Administrador) que la resuelve.
